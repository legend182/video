...
//	@Success		200	{object}	web.Response
//	@Router			/api/storage/v0/download [get]
func DownloadHandler1(c *gin.Context) {
	uidStr := c.Query("uid")
	online := c.Query("online")
	//文件名
	name := c.Query("name")
	...
	if !utils.Contains(online, []string{"0", "1"}) {
		lgLogger.WithContext(c).Error("online参数有误")
		web.ParamsError(c, "online参数有误")
		return
	}
	...
	if proxyFlag {
		...
		if !meta.MultiPart {
			...
			// 查找起始分片
			//startP_   lengthP_为通过start查找到的目标块的开始位置以及待获取的长度
			// index为当前块的下标   totalSize为当前块的起始位置在整个大文件下标的位置坐标
			index, totalSize := int64(0), int64(0)
			var startP, lengthP int64
			for {
				if totalSize == start {
			...
			for i := 0; i < utils.MultiPartDownload; i++ {
				...
				go func(i int, startP_, lengthP_ int64) {
					for {
						...
						// totalSize和start分别代表的整个文件中块起始位置和待获取位置坐标
						// 所以当前块的坐标大于start就说明当前块需要全部发送下载
						if totalSize >= start {
							startP_, lengthP_ = 0, multiPartInfoList[index].StorageSize
						}
						totalSize += multiPartInfoList[index].StorageSize
